...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="2.2",
        choices=["2.2", "2.3"],
        help="The CPE version to use: 2.2 or 2.3 (default: 2.2)",
    )
    parser.add_argument(
        "-q",
        "--query",
    ...
    with open(cpe_dict_filepath, encoding="utf8") as fin:
        ...
        for cpe_item in cpe_items:
            ...
            for i, cpe in enumerate((cpe22, cpe23)):
                ...
                cpe_elems = (
                    cpe_mod[7:].split(":") if i == 0 else cpe_mod[10:].split(":")
                )
        ...
        if zero_extend_versions:
            ...
            if version_match:
                alt_query = query.replace(
                    version_match.group(0), version_match.group(0) + ".0"
                )
                alt_queries_mapping[query].append(alt_query)
    ...
    if keep_data_in_memory:
        ...
        for cpe, indirect_cpe_tf, cpe_abs in CPE_TFS:
            for query in queries:
                ...
                inner_product = sum(
                    [cpe_tf[w] * query_tf[w] for w in intersecting_words]
                )
...
if __name__ == "__main__":
    ...
    if args.queries and not os.path.isfile(CPE_DATA_FILES[args.version]):
        if not SILENT:
            print(
                "[+] Running initial setup (might take a couple of minutes)",
                file=sys.stderr,
            )
        update(args.version)
